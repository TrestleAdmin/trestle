...
describe Trestle::Form::Builder, type: :helper do
  ...
  let(:object) { instance_double("Article", title: "Title", blob: { foo: "bar" }, errors: ActiveModel::Errors.new([])) }
  let(:options) { {} }
  ...
  describe "#json_text_area" do
    it "renders the container and hidden element with a label within a form group" do
      result = builder.json_text_area(:blob)

      expect(result).to have_tag(".form-group") do
        with_tag "label.control-label", text: "Blob", without: { class: "sr-only" }
        with_tag "input.form-control", with: { type: "hidden" }
        with_tag "div", with: { "data-enable-jsoneditor": true }
      end
    end
  end
end
