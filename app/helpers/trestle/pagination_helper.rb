module Trestle
  module PaginationHelper
    ...
    def page_entries_info(collection, options = {})
      ...
      total_count= collection.try(:total_count) || collection.count

      entry_name = options[:entry_name] || "entry"
      entry_name = entry_name.pluralize unless total_count == 1

      if collection.total_pages < 2
        t('trestle.helpers.page_entries_info.one_page.display_entries', entry_name: entry_name, count: total_count, default: "Displaying <strong>all %{count}</strong> %{entry_name}")
      else
        first = number_with_delimiter(collection.offset_value + 1)
        last  = number_with_delimiter((sum = collection.offset_value + collection.limit_value) > total_count ? total_count : sum)
